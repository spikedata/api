...
describe("full response pipeline = lambda-gw-client", function() {
  const shapes = Object.keys(API.shape).filter((x) => x.startsWith("lambda-gw"));
  ...
  for (const shape of shapes) {
    ...
    const testname = `${lambdaGwCode} request pipeline`;
    it(testname, async function() {
      try {
        ...
        if (lambdaGwShape.internal) {
          ...
          const examples = Object.keys(lambdaGwShape.examples);
