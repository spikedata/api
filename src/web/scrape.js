...
module.exports = async function(
  ...
  try {
    ...
    let accountsResponse = await spikeApi.accounts(APIKEY, USERKEY, loginResponse.sessionId, false);
    ...
    if (loginResponse && !autoClose) {
      ...
      let closeResponse = await spikeApi.close(APIKEY, USERKEY, loginResponse.sessionId);
...
async function loginInterimAbsPass(APIKEY, USERKEY, PASS, sessionId, requiredChars) {
  ...
  // inputs
  let data = [PASS[requiredChars[0]], PASS[requiredChars[1]], PASS[requiredChars[2]]];

  // request
  console.log(chalk.green("/login-interim-input (abs) ..."));
  let response = await spikeApi.loginInterimInputAbsPass(APIKEY, USERKEY, sessionId, false, data);
...
async function loginInterimStdOtp(APIKEY, USERKEY, sessionId) {
  ...
  let response = await spikeApi.loginInterimInputStdOtp(APIKEY, USERKEY, sessionId, false, data);
  if (response.type !== spikeApi.enums.TYPES.SUCCESS) {
...
async function loginInterimCapWait(APIKEY, USERKEY, sessionId) {
  ...
  let response = await spikeApi.loginInterimWait(APIKEY, USERKEY, sessionId, false);
  if (response.type !== spikeApi.enums.TYPES.SUCCESS) {
